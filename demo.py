...
from dust3r.utils.image import load_images, rgb, load_images_with_name
...
def get_reconstructed_scene(outdir, model, device, silent, image_size, filelist, schedule, niter, min_conf_thr,
    ...
    # print("camera parameters:", scene.get_im_poses(), scene.get_intrinsics())
    #save intrinsics as cameras.txt
    # with open(os.path.join(outdir, 'cameras.txt'), 'w') as f:
    #     for i, intrinsic in enumerate(scene.get_intrinsics()):
    #         f
    #save extrinsics as images.txt
...
#input: model, input_path, args
def get_pointcloud_cam_param(model, args):
    device = args.device
    image_size = args.image_size
    silent = args.silent
    
    imgs, file_name_list = load_images_with_name(args.input_path, size=image_size, verbose=not silent)
    if len(imgs) == 1:
        imgs = [imgs[0], copy.deepcopy(imgs[0])]
        imgs[1]['idx'] = 1
    pairs = make_pairs(imgs, scene_graph=args.scene_graph, prefilter=None, symmetrize=args.symmetrize)
    output = inference(pairs, model, device, batch_size=batch_size, verbose=not silent)
    ...
    with open(os.path.join(args.out_dir, 'intrinsics.txt'), 'w') as f:
        for i, intrinsic in enumerate(cam_intrinsics):
            f.write(f"{file_name_list[i]}\n")
            intrinsic = intrinsic.cpu().detach().numpy()
            for row in intrinsic:
                for item in row:
                    f.write(f"{item} ")
                f.write("\n")

    with open(os.path.join(args.out_dir, 'extrinsics.txt'), 'w') as f:
        for i, extrinsic in enumerate(cam_extrinsics):
            f.write(f"{file_name_list[i]}\n")
            extrinsic = extrinsic.cpu().detach().numpy()
...
if __name__ == '__main__':
    ...
    get_pointcloud_cam_param(model, args)
